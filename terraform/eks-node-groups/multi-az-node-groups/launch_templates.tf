...
module "launch_templates" {
  ...
  k8s_labels                = merge(local.node_groups_expanded[each.key]["labels"], try(each.value.k8s_labels, {}))
  k8s_taints                = concat(local.node_groups_expanded[each.key]["taints"], try(each.value.k8s_taints, []))
