...
locals {
  int_linux_default_user_data = var.create && (var.enable_bootstrap_user_data || var.user_data_template_path != "") ? base64encode(templatefile(
  ...
  linux = {
    user_data = try(data.cloudinit_config.linux_eks_managed_node_group.rendered, local.int_linux_default_user_data)
